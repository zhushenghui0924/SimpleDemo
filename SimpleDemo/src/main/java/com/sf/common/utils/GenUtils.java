...
public class GenUtils {
    ...
    public static void generatorCode(Map<String, String> table,
        ...
        tableBean.setComments(table.get("tableComment"));
        //表名转换成Java类名
        String className = tableToJava(tableBean.getTableName(), config.getString("tablePrefix"), config.getString("autoRemovePre"));
        tableBean.setCapitalClassName(className);
        tableBean.setLowercaseClassname(StringUtils.uncapitalize(className));
        ...
        for (Map<String, String> column : columns) {
            ...
            columnBean.setCapitalAttrName(attrName);
            columnBean.setLowercaseAttName(StringUtils.uncapitalize(attrName));
        ...
        map.put("className", tableBean.getCapitalClassName());
        map.put("classname", tableBean.getLowercaseClassname());
        ...
        for (String template : templates) {
            ...
            try {
                //添加到zip
                zip.putNextEntry(new ZipEntry(getFileName(template, tableBean.getLowercaseClassname(), tableBean.getCapitalClassName(), config.getString("package").substring(config.getString("package").lastIndexOf(".") + 1))));
                IOUtils.write(sw.toString(), zip, "UTF-8");
                IOUtils.closeQuietly(sw);
                zip.closeEntry();
            } catch (IOException e) {
                throw new BDException("渲染模板失败，表名：" + tableBean.getTableName(), e);
            }
        }
    }

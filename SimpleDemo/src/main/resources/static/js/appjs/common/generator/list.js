...
function batchCode() {
	...
	if (rows.length == 0) {
        swal("请选择要生成代码的表","至少选择一行!", "warning");
		return;
	}
    ...
    document.write("<form action='"+prefix + "/batchCode" + "' method=post name=batchCode style='display:none'>");
    document.write("<input type=hidden name='tables' value='"+JSON.stringify(tables)+"'/>");
    document.write("</form>");
    document.batchCode.submit();
